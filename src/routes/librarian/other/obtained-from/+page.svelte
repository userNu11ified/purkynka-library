<script lang="ts">
	...
	const obtainedFrom = $derived(librarianData.obtainedFrom.getArray());
	const books = $derived(librarianData.books.getArray());
	const usedDiscardReasonIds = $derived(new Set(books.map((v) => v.obtainedFromId)));

	const onEditClick = (obtainedFromId: number) => {
		tableView?.clearSelection();
		pushState('', { lookupEditorState: { type: 'obtainedFrom', id: obtainedFromId } });
	};

	const onDeleteClick = async (obtainedFromId: number) => {
		tableView?.clearSelection();

		const deleteResult = await librarianData.obtainedFrom.delete([obtainedFromId]);
	...
	const onMergeClick = (obtainedFromValues: [string, number][]) => {
		tableView?.clearSelection();
		pushState('', { mergeEditorState: { values: obtainedFromValues } });
	};

	mergeEditorSubmitCallbacks.registerCallback(async ([selectedIndex, mergedIndices]) => {
		const updateTo = obtainedFrom[selectedIndex].id;
		const toUpdate = mergedIndices
			.filter((v) => v !== selectedIndex)
			.map((index) => obtainedFrom[index].id);

		const booksToUpdate = books
			.filter((v) => v.obtainedFromId !== null && toUpdate.includes(v.obtainedFromId))
			.map((v) => ({ ...v, obtainedFromId: updateTo }));
		...
		const deleteResult = await librarianData.obtainedFrom.delete(toUpdate);
		if (Result.isError(deleteResult)) {
...
<TableView
	...
	persistentStateId="obtained-from-column-sizes"
	items={obtainedFrom}
