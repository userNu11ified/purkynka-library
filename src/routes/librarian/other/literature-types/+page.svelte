<script lang="ts">
	...
	const literatureTypes = $derived(librarianData.literatureTypes.getArray());
	const books = $derived(librarianData.books.getArray());
	const usedLiteratureTypeIds = $derived(new Set(books.map((v) => v.literatureTypeId)));

	const onEditClick = (literatureTypeId: number) => {
		tableView?.clearSelection();
		pushState('', {
			shorthandEditorState: { type: 'literatureType', id: literatureTypeId }
		});
	};

	const onDeleteClick = async (literatureTypeId: number) => {
		tableView?.clearSelection();

		const deleteResult = await librarianData.literatureTypes.delete([literatureTypeId]);
...
<TableView
	...
	persistentStateId="other-literature-types"
