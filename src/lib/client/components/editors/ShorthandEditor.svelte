<script lang="ts">
	...
	const shorthandEditorState = $derived(editorPageStates.shorthandEditorState!);

	const librarianData = LibrarianData.context.get();
	const editingData = $derived.by(() => {
		if (shorthandEditorState.type === 'literatureType') return librarianData.literatureTypes;
		else return librarianData.udc;
	});
	const editedValue = $derived(editingData.getValueByIdOrNull(shorthandEditorState.id)!);
	...
	const onSaveClick = async () => {
		...
		const newValue = {
			shortName: editorState.shortName.getParsedValue()!,
			longName: editorState.longName.getParsedValue()!
		};

		const patchResult = await editingData.patch({ ids: [id], newValue });
		if (Result.isError(patchResult)) {
			clientLogger.fatal('Failed to PATCH shorthand!', { error: patchResult.value });
			throw new Error();
		}
	...
	onMount(() => {
		editorState.id.setFromValue(shorthandEditorState.id);
		editorState.shortName.setFromValue(editedValue.shortName);
		editorState.longName.setFromValue(editedValue.longName);
	});
</script>
