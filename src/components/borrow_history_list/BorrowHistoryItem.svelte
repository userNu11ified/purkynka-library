<script lang="ts">
	...
	export let list_item: ListItem<BorrowHistoryListMappedItem>;
	$: id = list_item[0];
	$: item = list_item[1];
...
<div class="part center" class:even class:searched class:selected>{id + 1}</div>
...
<div class="part center" class:even class:searched class:selected>
	{item.permanent && item.return_date === null ? 'Trvale' : item.return_date ? format_date(item.return_date) : '—'}
</div>
<div class="part center" class:even class:searched class:selected>{item.permanent ? 'Ano' : 'Ne'}</div>
